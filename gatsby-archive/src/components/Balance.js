...
const Balance = () => {
  const [address, setAddress] = usePersistantState('');
  const [balance, setBalance] = React.useState(null);
  const [isValid, setIsValid] = React.useState(false);

  // TODO add a check to prod wss + error boundary?
  const [websocket, socketState] = useWebSocket('wss://hyper.bisafe.net/web-socket/', msg => {
